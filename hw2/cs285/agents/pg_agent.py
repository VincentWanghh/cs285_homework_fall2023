...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        #assert type(q_values) == " <class 'numpy.ndarray'>", 'q_values type is {}'.format(type(q_values))
        #assert type(advantages) == 'np.ndarray', 'advantages type is {}'.format(type(advantages))
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    advantages[i] = delta_t + \
                        self.gamma * self.gae_lambda * advantages[i+1] * (1 - terminals[i])
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        #list_of_discounted_returns = np.full(len(rewards), sum_rewards)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        list_of_discounted_reward_to_go = []
        T = len(rewards)
        for t in range(T):
            list_of_discounted_reward_to_go.append(np.sum([self.gamma**(tPrime - t) * rewards[tPrime] for tPrime in range(t, T)]))
        return list_of_discounted_reward_to_go
